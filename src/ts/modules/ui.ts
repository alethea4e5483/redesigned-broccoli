...
interface EndpointData extends Endpoint {
  [key: string]: any
}

function getEndpointData(): EndpointData[] {
  return (window as any).endpointData || []
}

function getIdentityToken(): string | null {
  return (window as any).identityToken || stateManager.getIdentityToken()
}
...
export function handleFormSubmit(
  ...
  const body = buildRequestBody(ep, formElements, _e)
  ...
  if (responseResultDiv) {
    ...
    const copyBtn = document.getElementById('copy-response-btn') as HTMLButtonElement | null
  ...
  if (typeof (window as any).showMobileResponsePanel === 'function') {
    (window as any).showMobileResponsePanel()
  }

  sendRequest(url, identityToken, MsgType, RespType, body, opts)
}

export function renderForm(ep: Endpoint): void {
  console.log('Rendering form for endpoint:', ep)

  if (typeof (window as any).setMobileTabsVisible === 'function') {
    console.log('Setting mobile tabs visible')
    ;(window as any).setMobileTabsVisible(true)
  }
  ...
  const form = document.getElementById('apiForm') as HTMLFormElement | null
  if (!form) return

  form.onsubmit = function (this: HTMLFormElement, e: SubmitEvent) {
    e.preventDefault()
    ...
    if (ep.type === 'rpc') {
      try {
        ...
        MsgType = ('proto.' + ep.request)
          .split('.')
          .reduce((o: any, k: string) => o?.[k], (window as any))
        ...
        alert('Protobuf-Types could not be loaded: ' + errMsg)
        return
      }
    }
...
export async function getProtoType(path: string): Promise<any> {
  if (!(window as any).protobufRoot) {
    console.error('Protobuf root is not loaded')
    return null
  }

  try {
    const type = (window as any).protobufRoot.lookupType(
      path.replace(/^proto\./, '')
    )
