...
export function createMetadataDropdown(
...
export function renderMetadataInputs(
  ...
  metadataKeys.forEach((key) => {
    ...
    if (Array.isArray(meta.rules?.options)) {
      ...
      meta.rules.options.forEach((opt: string) => {
        const option = document.createElement('option')
        option.value = opt
        option.textContent = opt
        input.appendChild(option)
      })
      if (prevValues[input.name]) {
        (input as HTMLSelectElement).value = prevValues[input.name]
      }
    } else {
      const inputType = meta.rules?.type === 'int' ? 'number' : 'text'
      ...
      input.style.width = '350px'
      if (prevValues[input.name]) {
        (input as HTMLInputElement).value = prevValues[input.name]
      }
    }
      ...
      if (idx !== -1) {
        ...
        if (dropdownRow) {
          ...
          dropdownRow.appendChild(
            createMetadataDropdown(metadataList, () => {}, metadataKeys)
          )
        }
      }
    }
